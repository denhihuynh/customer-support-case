...
const hasRequiredFields = (jsonBody) => {
  return typeof jsonBody.supportAgentId === "string"
};
...
module.exports = functions.http("main", async (req, res) => {
  ...
  try {
    const response = await firestore.collection("SupportAgent").doc(body.supportAgentId).delete();
    console.log("Successfully removed", JSON.stringify(response, null, 2));
    res.send(200);
  } catch (error) {
